...
export function heartbeatService(db: Db) {
  ...
  async function releaseIssueExecutionAndPromote(run: typeof heartbeatRuns.$inferSelect) {
    const promotedRuns = await db.transaction(async (tx) => {
      ...
      if (lockedIssues.length === 0) return [] as Array<typeof heartbeatRuns.$inferSelect>;
      ...
      const promoted: Array<typeof heartbeatRuns.$inferSelect> = [];
    ...
    if (promotedRuns.length === 0) return;
